...
- [x] 可進入的建築物：科研大樓、六教、光華館（4 層，1、2F「綠光庭園」餐廳區有門，兩層都有餐廳人員）、行政大樓、三教、一教、二教、四教、共同科館、綜合科館、設計館、圖書館（B1～3F，各樓層服務分區照官網「樓層配置」）、先鋋大樓（忠孝東路對面，過馬路才到得了，14 層）
...
- [x] 正校門正對面過忠孝東路的街廓：加了一段人行穿越道，過馬路才到得了先鋋大樓（官網「本校導覽」：先鋋國際研發大樓在正門對面）
- [x] 先鋋大樓的大門面向馬路（北側），跟其他建築物「大門在南側」的方向相反，走出大門後會站在門口正上方、面向上
- [x] 修正一個既有 bug：建築物室內「入口降落點」跟「室內出口」原本只隔一格，一進門按一下方向鍵就會立刻被彈回門外；這個問題影響所有建築物，不是先鋋大樓特有的，已修正並補上迴歸測試
...
新生南路                                                                建國南路
  │ ┌────────┐┌────┐┌──────────┐  ┌────────┐┌──────────┐           │
  │ │ 光華館 ││六教││ 科研大樓  │  │校友會館│ 化學工程館 分子科學   │
  │ │(綠光庭園││    ││(系辦3F331)│  └────────┘ 工程館             │
新生校門══════════════════════════ 北側道路（東西向）══════════════════│
  │ ┌──────┐┌──────┐┌──────┐  ┌────┐┌────┐        ┌────────────┐│
  │ │土木館││材資館││設計館│  │一教││二教│        │學生活動中心 ││
  │ └──────┘└──────┘└──────┘  └────┘└────┘        └────────────┘│
  │                    ┌───┐┌────┐┌──┐                          │
  │                    │校史││三教││紅樓                          │
  │                    │館  ││    ││樓│                          ├建國側門
...
（示意圖只求相對位置與分區大致正確，格子大小、對齊不代表實際比例。）
...
先鋋大樓：正式名稱「先鋋國際研發大樓」，位在正門對面（忠孝東路），2022 年啟用，需要過馬路才能到；遊戲裡在正校門南側加了一段人行穿越道連接，大門面向北（忠孝東路），跟其他建築物「大門在南側」的方向相反。
